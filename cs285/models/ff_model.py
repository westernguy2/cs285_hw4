...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        next_obs_pred = obs_unnormalized + ((delta_pred_normalized * ptu.from_numpy(delta_std)) + ptu.from_numpy(delta_mean))# TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction = self(ptu.from_numpy(obs), ptu.from_numpy(acs), **data_statistics)[0]# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        loss = self.loss(ptu.from_numpy(target), self(ptu.from_numpy(observations), ptu.from_numpy(actions), **data_statistics)[1])# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
